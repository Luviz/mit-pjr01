...
public class Persons {
	 ...
	 *            - city
	 */
	public void Add(String fName, String sName, int length, String add,
			String zip, String city) {

		main.add(new Person(sigGen(fName, sName), fName, sName, length,
				new Address(add, zip, city)));
		updateTmp();
	}
	...
	private String sigGen(String fName, String sName) {
		...
		String ret = new String(sig).replace(" ", "x"); // this will fix names
														// like Bo Erik
	...
	private void sortByName() {
		...
		int minIx = 0;
		while (!t.isEmpty()) {
			String tocheckM = new String(t.get(0).getfName() + " "
					+ t.get(0).getsName());
			for (int j = 0; j < t.size(); j++) {
				String check = new String(t.get(j).getfName() + " "
						+ t.get(j).getsName());
				if (tocheckM.compareTo(check) < 0) {
					// min < i
				} else {
					minIx = j;
					tocheckM = new String(t.get(minIx).getfName() + " "
							+ t.get(minIx).getsName());
				}
			}
		...
		/*
		 * DEBUG for (Person person : tmp) { System.out.println(person); } //
		 */
	}
	...
	// password to e01 'hello'
	public void encrypt(String url, String seed) throws IOException {
		Encryptor e = new Encryptor(seed);
		BufferedWriter output = new BufferedWriter(new FileWriter(url));
		for (Person p : main) {
			output.write(e.encrypt(p.toWrite()));
		}
		output.close();
	}

	public void decrypt(String url, String seed) throws IOException {
		Encryptor e = new Encryptor(seed);
		BufferedReader input = new BufferedReader(new FileReader(url));
		String in = "";
		while ((in = input.readLine()) != null) {
			in = e.decrypt(in); // MAKE SURE to have the right seed else it will
								// crash!
			main.add(new Person(in));
			System.out.println(in);
		}
